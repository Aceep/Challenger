...
- The cookie is written **only** by `switchChallengeAction` (`app/(player)/help/actions.ts`), reachable from Aide › Édition, the admin rail and the editions table of Admin › Défi; it lasts a year and is deleted on sign-out. Landing after a switch is capped by `switchLanding`: the admin desk only for an ORGANIZER of the target. The current edition is spelled out permanently in both shells (`EditionBar`, the « Plus » sheet, the rail block) — its colour paints `--edition`.
