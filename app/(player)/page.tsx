...
export default async function HomePage() {
  ...
  const graphics = books.filter((b) => b.type === "GRAPHIQUE").length;
    ...
    <main className="flex flex-1 flex-col gap-6 p-5">
      ...
      {team ? (
        ...
        >
          ...
          <p className="text-4xl font-black">{fmtPoints(score)} pts</p>
      ...
      <section className="grid grid-cols-2 gap-3">
        ...
        <div className="rounded-xl bg-white p-4 shadow-sm dark:bg-slate-900">
          ...
          <p className="text-2xl font-bold">{fmtPoints(myPoints)}</p>
        </div>
      </section>
